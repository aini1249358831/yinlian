...
public class PayServiceImpl {
    ...
    public String payWeb(HttpServletRequest req, HttpServletResponse resp){
        ...
        /***商户接入参数***/
        requestData.put("merId", "777290058166588");    	          			  //商户号码，请改成自己申请的正式商户号或者open上注册得来的777测试商户号
    ...
    public  String payCode(HttpServletRequest req, HttpServletResponse resp) {
        resp.setContentType("text/html; charset="+ DemoBase.encoding);
        ...
        String requestFrontUrl = SDKConfig.getConfig().getFrontRequestUrl();  //获取请求银联的前台地址：对应属性文件acp_sdk.properties文件中的acpsdk.frontTransUrl
        Map<String, String> rspData = AcpService.post(submitFromData,requestFrontUrl,DemoBase.encoding);  //发送请求报文并接受同步应答（默认连接超时时间30秒，读取返回结果超时时间30秒）;这里调用signData之后，调用submitUrl之前不能对submitFromData中的键值对做任何修改，如果修改会导致验签不通过
        ...
        String reqMessage = DemoBase.genHtmlResult(requestData);
